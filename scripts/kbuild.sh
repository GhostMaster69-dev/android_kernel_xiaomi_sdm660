...
build_kernel() {
    ...
    make ARCH=arm64 CC=$COMPILER_NAME LD=$LD_NAME CROSS_COMPILE=$CROSS_COMPILE_ARM64 CROSS_COMPILE_ARM32=$CROSS_COMPILE_ARM32 $CMDS O=$OUT_DIR -j$(nproc --all) |& tee -a $HOME/build/$target-build$BUILD.txt
    BUILD_END=$(date +"%s")
...
for target in "${targets[@]}"; do
    ...
    DEFCONFIG="${target}_defconfig kernelsu-next.config"
